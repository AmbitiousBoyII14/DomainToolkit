...
export interface PlanConfig {
  ...
  dailyScans: number; historyLimit: number; allowedTools: string[]
  canExport: boolean; canBulkScan: boolean; hasApiKey: boolean; hasPrioritySupport: boolean
  badge?: string
}
...
export const PLAN_CONFIGS: Record<Plan, PlanConfig> = {
  ...
  pro: {
    ...
    dailyScans: 100, historyLimit: 500, allowedTools: 'all' as unknown as string[],
  ...
  business: {
    ...
    dailyScans: -1, historyLimit: -1, allowedTools: 'all' as unknown as string[],
...
export interface User {
  ...
  plan: Plan; provider: 'email' | 'google' | 'facebook'
...
export function getScanCount(): number {
  ...
  try {
    const e: UsageEntry = JSON.parse(localStorage.getItem(SCAN_KEY) ?? '{"date":"","count":0}')
    return e.date !== todayStr() ? 0 : e.count
  } catch { return 0 }
}
export function incrementScanCount(): void {
  ...
  try {
    const e: UsageEntry = JSON.parse(localStorage.getItem(SCAN_KEY) ?? '{"date":"","count":0}')
    const c = e.date === today ? e.count + 1 : 1
    localStorage.setItem(SCAN_KEY, JSON.stringify({ date: today, count: c }))
  } catch { /* ignore */ }
}
...
export function getAllUsers(): User[] {
  if (typeof window === 'undefined') return []
  try { return JSON.parse(localStorage.getItem(USERS_DB_KEY) ?? '[]') as User[] }
  catch { return [] }
}
...
async function syncUserToServer(user: User) {
  try { await fetch('/api/users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user }) }) }
  catch { /* offline OK */ }
}

export async function upgradePlanServer(user: User, plan: Plan): Promise<boolean> {
  try {
    const res = await fetch('/api/users', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: user.email, plan }) })
...
export function upgradePlanLocal(user: User, plan: Plan): User {
  const updated = { ...user, plan }; storeUser(updated)
  const db = getAllUsers(); const idx = db.findIndex(u => u.email === user.email)
...
function getFirstName(email: string): string { return email.split('@')[0].replace(/[._-]/g, ' ').split(' ')[0] }
...
export function signUpEmail(name: string, email: string, _password: string): AuthResult {
  ...
  if (db.find(u => u.email === email)) return { user: null, error: 'An account with this email already exists. Please sign in.' }
  const user: User = { id: generateId(), name: name.trim() || getFirstName(email), email: email.trim().toLowerCase(), plan: 'free', provider: 'email', createdAt: Date.now() }
  storeUser(user)
...
export function signInEmail(email: string, _password: string): AuthResult {
  ...
  if (!existing) return { user: null, error: 'No account found with this email. Please sign up first.' }
  storeUser(existing)
...
export function signInOAuth(provider: 'google' | 'facebook', name: string, email: string): AuthResult {
  const db = getAllUsers()
  const existing = db.find(u => u.email === email.trim().toLowerCase())
  if (existing) { if (existing.provider !== provider) existing.provider = provider; storeUser(existing); return { user: existing, error: null } }
  const user: User = { id: generateId(), name, email: email.trim().toLowerCase(), plan: 'free', provider, createdAt: Date.now() }
  storeUser(user)
  return { user, error: null }
}
...
export async function forgotPassword(email: string): Promise<ForgotResult> {
  try {
    ...
    return { success: data.success || false, message: data.message || 'Something went wrong', resetUrl: data.resetUrl }
  } catch {
...
export async function resetPassword(token: string, email: string, newPassword: string): Promise<ForgotResult> {
  try {
    const res = await fetch('/api/auth/reset-password', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ token, email: email.trim().toLowerCase(), newPassword }),
    })
    const data = await res.json()
    return { success: data.success || false, message: data.message || data.error || 'Something went wrong' }
  } catch {
    return { success: false, message: 'Network error. Please try again.' }
  }
}
...
export interface AuthContextValue {
  ...
  openAuthModal: (mode?: 'signin' | 'signup') => void; openPricing: () => void
}

export const AuthContext = createContext<AuthContextValue>({
  user: null, setUser: () => {}, openAuthModal: () => {}, openPricing: () => {},
})
