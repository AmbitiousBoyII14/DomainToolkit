...
async function httpFetch(url: string, timeoutMs = 8000): Promise<{
  status: number
  ...
  return new Promise((resolve, reject) => {
    const socket = tls.connect({ host, port: 443, servername: host, rejectUnauthorized: false, timeout: 8000 }, () => {
      ...
      resolve({
        ...
        'Days Remaining': daysLeft > 0 ? String(daysLeft) : 'Expired',
        'TLS Version':  proto,
        'Cipher':       (socket.getCipher() as any)?.name ?? '',
        'HSTS':         'Check HTTP headers',
        'Subject Alt Names': (cert.subjectaltname ?? '').replace(/DNS:/g, '').trim(),
      })
    })
  ...
  try {
    switch (tool) {
      ...
      case 'domain-overview': {
        const [aRes, nsRes, mxRes] = await Promise.allSettled([
          dns.resolve4(host),
          dns.resolveNs(host),
          dns.resolveMx(host),
        ])
        ...
        if (httpData) {
          fields['HTTP Status'] = String(httpData.status)
          fields['Server'] = httpData.headers['server'] ?? 'unknown'
          fields['Content-Type'] = httpData.headers['content-type'] ?? 'unknown'
          fields['HTTPS'] = httpData.finalUrl.startsWith('https') ? 'Yes' : 'No'
        }
      ...
      case 'http': {
        ...
        let protocol = 'https'
        try {
          httpData = await httpFetch(`https://${host}`)
        } catch {
          try {
            httpData = await httpFetch(`http://${host}`)
            protocol = 'http'
          } catch { /* ignore */ }
        }
        ...
        const fields: Record<string, string> = {
          Domain: host,
          'HTTP Status': String(httpData.status),
          Protocol: protocol.toUpperCase(),
          Server: httpData.headers['server'] ?? 'Not set',
          'Content-Type': httpData.headers['content-type'] ?? 'Not set',
          'X-Powered-By': httpData.headers['x-powered-by'] ?? 'Not set',
          'Cache-Control': httpData.headers['cache-control'] ?? 'Not set',
          'Content-Encoding': httpData.headers['content-encoding'] ?? 'Not set',
          'Transfer-Encoding': httpData.headers['transfer-encoding'] ?? 'Not set',
          'Set-Cookie': httpData.headers['set-cookie'] ?? 'Not set',
        }
          ...
          if (res.ok) {
            ...
            if (Array.isArray(data?.events)) {
              for (const e of data.events) {
                if (e.eventAction === 'registration') events['Registered'] = e.eventDate ?? ''
                if (e.eventAction === 'expiration')   events['Expires']    = e.eventDate ?? ''
                if (e.eventAction === 'last changed') events['Updated']    = e.eventDate ?? ''
              }
            }
            Object.assign(whoisData, events)
            if (Array.isArray(data?.nameservers)) {
              whoisData['Nameservers'] = data.nameservers.map((n: any) => n.ldhName).join(', ')
            }
          }
        } catch { /* use fallback */ }
      ...
      case 'subdomains': {
        ...
        const results = await Promise.allSettled(
          common.map(sub => dns.resolve4(`${sub}.${host}`).then(() => `${sub}.${host}`))
        )
        ...
        const fields: Record<string, string> = {
          ...
          'Scanned Prefixes': String(common.length),
          ...(found.length ? { 'Found': found.join(', ') } : { Note: 'No common subdomains resolved' }),
        }
      ...
      case 'websocket': {
        ...
        for (const path of paths) {
          try {
            const res = await httpFetch(`https://${host}${path}`, 5000)
            const upgraded = res.headers['upgrade']?.toLowerCase() === 'websocket'
            if (upgraded) detected = true
            rows.push({ Path: path, Status: String(res.status), Upgraded: upgraded ? 'Yes' : 'No' })
          } catch {
            rows.push({ Path: path, Status: 'Error', Upgraded: 'No' })
          }
        }

        const fields: Record<string, string> = {
          Domain: host,
          'WebSocket Found': detected ? 'Yes' : 'No',
          'Paths Checked': String(paths.length),
          'Paths with WS': String(rows.filter(r => r.Upgraded === 'Yes').length),
        }
      ...
      case 'ping': {
        ...
        let statusCode = 0
        try {
          const res = await httpFetch(`https://${host}`, 6000)
          latency = Date.now() - start
          reachable = true
          statusCode = res.status
        } catch {
          try {
            const res2 = await httpFetch(`http://${host}`, 6000)
            latency = Date.now() - start
            reachable = true
        ...
        return NextResponse.json({
          success: true,
          type: 'Ping / Reachability',
          fields: {
            Host: host,
            Reachable: reachable ? 'Yes' : 'No',
            'Response Time': reachable ? `${latency} ms` : 'N/A',
            'HTTP Status': reachable ? String(statusCode) : 'N/A',
          },
        })
      }
      ...
      case 'port-scan': {
        ...
        await Promise.allSettled(
          COMMON_PORTS.map(([port, service]) =>
            new Promise<void>((resolve) => {
              ...
              sock.on('connect', () => { rows.push({ Port: String(port), Service: service, Status: 'open' }); sock.destroy(); resolve() })
        ...
        return NextResponse.json({
          success: true,
          type: 'Port Scanner',
          fields: { Host: host, 'Open Ports': String(open.length), 'Ports Scanned': String(COMMON_PORTS.length) },
          rows,
        })
      }
      ...
      case 'http-response-time': {
        ...
        for (let i = 0; i < 3; i++) {
          const t = Date.now()
          try { await httpFetch(`https://${host}`, 6000); times.push(Date.now() - t) } catch { /* ignore */ }
        }
        const avg = times.length ? Math.round(times.reduce((a, b) => a + b, 0) / times.length) : null

        return NextResponse.json({
          success: true,
          type: 'HTTP Response Time',
          fields: {
            Host: host,
            'Avg Response': avg !== null ? `${avg} ms` : 'Unreachable',
            'Samples': String(times.length),
            'Min': times.length ? `${Math.min(...times)} ms` : 'N/A',
            'Max': times.length ? `${Math.max(...times)} ms` : 'N/A',
          },
        })
      }
      ...
      case 'hosting': {
        ...
        const xAmzHeader = headers['x-amz-cf-id'] ?? headers['x-amz-id-2']
        ...
        else if (viaHeader.includes('akamai'))            { provider = 'Akamai'; cdn = 'Akamai' }
        ...
        const fields: Record<string, string> = {
          Domain: host,
          'HTTP Status': httpData ? String(httpData.status) : 'N/A',
          Provider: provider,
          CDN: cdn,
          'Server Header': headers['server'] ?? 'Not set',
          'X-Powered-By': headers['x-powered-by'] ?? 'Not set',
          'Via': headers['via'] ?? 'Not set',
        }
      ...
      case 'security-headers': {
        ...
        const CHECKED_HEADERS: [string, string][] = [
        ...
        const fields: Record<string, string> = { Domain: host }
        let score = 0
        for (const [label, key] of CHECKED_HEADERS) {
          const val = h[key]
          fields[label] = val ?? 'Missing'
          if (val) score += 10
        }
      ...
      case 'all-in-one': {
        ...
        // DNS
        try {
          const [a, ns, mx] = await Promise.allSettled([dns.resolve4(host), dns.resolveNs(host), dns.resolveMx(host)])
          if (a.status === 'fulfilled') allFields['IP Address'] = a.value[0] ?? ''
          if (ns.status === 'fulfilled') allFields['Nameservers'] = ns.value.join(', ')
          if (mx.status === 'fulfilled') allFields['MX Records'] = mx.value.map(r => r.exchange).join(', ')
        } catch { /* ignore */ }
        ...
        try {
          httpData = await httpFetch(`https://${host}`)
          allFields['HTTP Status'] = String(httpData.status)
          allFields['Server'] = httpData.headers['server'] ?? 'unknown'
          allFields['HTTPS'] = 'Yes'
        } catch { /* ignore */ }
        ...
        try {
          const certFields = await sslCheck(host)
          Object.assign(allFields, certFields)
        } catch { /* ignore */ }

        // Security headers
        if (httpData) {
          const h = httpData.headers
          allFields['CSP'] = h['content-security-policy'] ? 'Present' : 'Missing'
          allFields['HSTS'] = h['strict-transport-security'] ? 'Present' : 'Missing'
          allFields['X-Frame-Options'] = h['x-frame-options'] ? 'Present' : 'Missing'
          allFields['X-Content-Type-Options'] = h['x-content-type-options'] ? 'Present' : 'Missing'
          allFields['Referrer-Policy'] = h['referrer-policy'] ? 'Present' : 'Missing'
        }

        // Hosting
        if (httpData) {
          const serverHeader = (httpData.headers['server'] ?? '').toLowerCase()
          const cfRay = httpData.headers['cf-ray']
          allFields['Provider'] = cfRay ? 'Cloudflare' : httpData.headers['x-vercel-id'] ? 'Vercel' : httpData.headers['x-nf-request-id'] ? 'Netlify' : serverHeader || 'Unknown'
        }
