 ...
 */
public class SSLToolsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        root.addView(UiKit.h1(getActivity(), "SSL / TLS Tools"));
        root.addView(UiKit.gap(getActivity(), 4));
        root.addView(UiKit.caption(getActivity(), "Certificate info, chain trust, TLS version & ciphers."));
        ...
        runBtn = UiKit.primaryButton(getActivity(), "Check SSL");
    ...
    private void startScan() {
        ...
        task = NetworkUtils.sslCertificate(domain, new NetworkUtils.Callback() {
    ...
    private void renderResult(ScanResult r) {
        // Trust verdict pill
        String trusted = r.getField("Trusted");
        if (!TextUtils.isEmpty(trusted)) {
            boolean ok = "Yes".equalsIgnoreCase(trusted) || "true".equalsIgnoreCase(trusted);
            int pillColor = ok ? Palette.success() : Palette.error();
            String pillText = ok ? "TRUSTED" : "UNTRUSTED";
        ...
        hdr.addView(UiKit.h2(getActivity(), "Certificate Details"));
        card.addView(hdr);
        card.addView(UiKit.divider(getActivity()));

        for (Map.Entry<String, String> e : r.getAllFields().entrySet()) {
            if ("Trusted".equalsIgnoreCase(e.getKey())) continue; // already shown as pill
    ...
    private void showError(String msg) {
        ...
        TextView tv = UiKit.body(getActivity(), TextUtils.isEmpty(msg) ? "SSL check failed." : msg);
    ...
    private void copyToClipboard(ScanResult r) {
        ...
        sb.append("SSL Check – ").append(r.getTarget()).append("\n\n");
        for (Map.Entry<String, String> e : r.getAllFields().entrySet()) {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("ssl result", sb.toString()));
