...
export function FavoritesView() {
  ...
  const handleUnstar = (id: string) => { toggleFavorite(id); reload() }
  return (<PageWrapper><PageHeading title="Favorites" subtitle="Starred scans you want to keep handy" />{favorites.length === 0 ? <EmptyState title="No favorites yet" description="Star any scan from the History screen." /> : (<div className="space-y-2">{favorites.map(item => (<FavoriteItem key={item.id} item={item} onUnstar={handleUnstar} />))}</div>)}</PageWrapper>)
}
