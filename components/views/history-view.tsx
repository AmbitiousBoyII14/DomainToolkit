...
export function HistoryView() {
  ...
  const reload = useCallback(() => {
    ...
    if (!query.trim()) { setHistory(all); return }
    const q = query.toLowerCase()
    setHistory(all.filter(r => r.target.toLowerCase().includes(q) || r.type.toLowerCase().includes(q)))
  }, [query])
  ...
  const handleClear = () => {
    if (confirm('Clear all history? Starred items will be kept.')) {
      clearHistory()
      reload()
    }
  }
    ...
    <PageWrapper>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{total} scan{total !== 1 ? 's' : ''} recorded</p>
        </div>
        {total > 0 && (
          <button
            onClick={handleClear}
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        {item.isFavorite && <Pill variant="warning">STARRED</Pill>}
      </div>
      <div className="flex items-center gap-3">
        <button
          onClick={() => onToggleFav(item.id)}
          className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-warning-foreground transition-colors"
          aria-label={item.isFavorite ? 'Unstar' : 'Star'}
        >
          {item.isFavorite ? <StarOff size={13} /> : <Star size={13} />}
