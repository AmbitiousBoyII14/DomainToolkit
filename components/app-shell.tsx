...
export function AppShell() {
  ...
  useEffect(() => { const stored = getStoredUser(); if (stored) setUserState(stored) }, [])
  const setUser = useCallback((u: User | null) => { setUserState(u); if (u) storeUser(u) }, [])
  const openAuthModal = useCallback((mode: 'signin' | 'signup' = 'signup') => { setAuthModalMode(mode); setAuthModalOpen(true) }, [])
  ...
  return (<AuthContext.Provider value={authCtx}><div className="flex h-screen overflow-hidden bg-background"><Sidebar activeId={activeId} onNavigate={navigate} isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />{sidebarOpen && <div className="fixed inset-0 z-30 bg-black/50 lg:hidden" onClick={() => setSidebarOpen(false)} />}<div className="flex flex-col flex-1 min-w-0 overflow-hidden"><Toolbar title={currentNav?.label ?? 'Domain Toolkit Pro'} onMenuClick={() => setSidebarOpen(true)} /><main className="flex-1 overflow-y-auto"><ActiveView activeId={activeId} navigate={navigate} onScanSaved={onScanSaved} historyVersion={historyVersion} /></main></div><AuthModal isOpen={authModalOpen} initialMode={authModalMode} onClose={() => setAuthModalOpen(false)} onSuccess={(u) => { setUser(u); setAuthModalOpen(false) }} /><ContactFAB /></div></AuthContext.Provider>)
}

function ActiveView({ activeId, navigate, onScanSaved, historyVersion }: { activeId: ToolId; navigate: (id: ToolId) => void; onScanSaved: () => void; historyVersion: number }) {
  switch (activeId) {
