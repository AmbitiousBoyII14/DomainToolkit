...
function SidebarContent({ activeId, onNavigate }: { activeId: ToolId; onNavigate: (id: ToolId) => void }) {
  ...
  return (<div className="flex flex-col h-full">
    <div className="px-5 py-5 border-b border-sidebar-border hidden lg:block">
      <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary to-violet-500 flex items-center justify-center shrink-0"><Globe size={17} className="text-white" /></div><div><p className="font-bold text-sm text-sidebar-fg">Domain Toolkit</p><p className="text-[11px] text-muted-foreground">Professional</p></div></div>
    </div>
    <nav className="flex-1 overflow-y-auto py-3 px-2.5 space-y-0.5">
      ...
      {mainItems.filter(i => toolIds.includes(i.id)).map(item => { const Icon = ICONS[item.icon] ?? Globe; const isActive = item.id === activeId
        return (<button key={item.id} onClick={() => onNavigate(item.id)} className={cn('w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-[13px] transition-all duration-150', isActive ? 'bg-sidebar-accent text-sidebar-accent-fg font-semibold shadow-sm' : 'text-sidebar-fg/70 hover:text-sidebar-fg hover:bg-sidebar-accent/50')}><Icon size={17} className={isActive ? 'text-sidebar-accent-fg' : 'text-muted-foreground'} /><span className="truncate">{item.label}</span>{item.premium && <span className="ml-auto text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-amber-100 text-amber-600 uppercase">PRO</span>}</button>)
      })}
    ...
    <div className="px-3 py-3 border-t border-sidebar-border space-y-1.5">
      {user ? (user.plan === 'free' ? <button onClick={() => onNavigate('pricing')} className={cn('w-full flex items-center gap-2 px-3 py-2 rounded-xl text-left text-xs font-semibold transition-all', activeId === 'pricing' ? 'gradient-btn' : 'bg-accent text-accent-foreground hover:bg-accent/80')}><Crown size={14} /><span>Upgrade to Pro</span></button> : <div className="flex items-center gap-2 px-3 py-2"><span className={cn('px-2 py-0.5 rounded-full text-[10px] font-bold', user.plan === 'business' ? 'bg-amber-100 text-amber-700' : 'bg-indigo-100 text-indigo-700')}>{user.plan.toUpperCase()}</span><span className="text-xs text-muted-foreground truncate">{user.name}</span></div>) : <button onClick={() => onNavigate('pricing')} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-left text-xs font-semibold bg-accent text-accent-foreground hover:bg-accent/80"><Crown size={14} /><span>View Plans</span></button>}
